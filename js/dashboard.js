...
function loadTasks(){
  ...
  tasks.filter(t=>t.user===user).forEach((t,i)=>{
    ...
    let assignments="";
    if(t.assignments){ // opdrachten per vak
    ...
    div.innerHTML=`
      <div>
        <b>${t.subject}</b> (${t.date}) ${t.examWeight?`[Exam ${t.examWeight}%]`:""}<br>
        ${t.task}<br>
        ${assignments}
      </div>
...
function addTask(){
  ...
  let assignments=[];
  if(num){
    assignments=num.split(",").map(n=>({name:n.trim(),done:false}));
  }
...
function toggleAssignment(taskIndex,assignIndex){
  ...
  // Als alle opdrachten gedaan, markeer hoofdtaak als done
